...
import { deleteInterview } from "@/components/interview/api";
...
export default function CompanyDetailPage() {
  ...
  const {
    ...
    isLoading: esLoading,
    deleteEntry,
  } = useCompanyESEntries(companyId);
  // APIからJobEventsデータを取得
  const { jobEvents, isLoading: jobEventsLoading } =
    useCompanyJobEvents(companyId);
  // APIから面接ログを取得
  const { logs: interviewLogs, isLoading: interviewLogsLoading } =
    useCompanyInterviewLogs(companyId);
  ...
  useEffect(() => {
    if (jobEvents.length > 0 && company) {
      const convertedEvents = jobEvents.map((jobEvent) =>
        jobEventToEvent(jobEvent, company.name)
      );
        ...
        <div className="max-w-6xl mx-auto space-y-6">
          ...
          <CompanyRelatedDataTabs
            ...
            onNavigateToInterview={() =>
              router.push(`/interview/new?companyId=${companyId}`)
            }
