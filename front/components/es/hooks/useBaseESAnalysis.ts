...
export interface BaseESAnalysisResult {
  summary: string
  advice: string
  adviceItems: AdviceItem[]
}

export function useBaseESAnalysis() {
  const [isAnalyzing, setIsAnalyzing] = useState(false)

  const analyzeContent = async (content: string): Promise<BaseESAnalysisResult> => {
    setIsAnalyzing(true)

    try {
      const response = await apiClient.post('/users/base-es/analyze', { content });
