...
export function useESGeneration() {
  ...
  const generateContent = useCallback(async (baseES: string, companyDescription: string, esTitle: string): Promise<ESGenerationResult> => {
    ...
    try {
      ...
      const result = await generateESContent(baseES, companyDescription, esTitle)
